...
CONTROLLER="application-cloudflarerecord"
...
async def application_cloudflarerecord_idx(
    namespace,
    body,
    **_,
):
...
@kopf.on.probe(
    id=application_cloudflarerecord_idx.__name__,
)
async def application_cloudflarerecord_probe(
    application_cloudflarerecord_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o
            for o in application_cloudflarerecord_idx[namespace]
        }
        for namespace in application_cloudflarerecord_idx
    }
...
async def application_cloudflarerecord(
    namespace,
    body,
    patch,
    **kwargs,
    ...
    body = (
        load(
            env.get_template("cloudflarerecord.yaml.j2").render(
                namespace=namespace,
                selector=label.but(
                    LABEL_CONTROLLER,
                    LABEL_INSTANCE,
