...
CONTROLLER="application-ingressroutetcp"
...
async def application_ingressroutetcp_idx(
    namespace,
    body,
    **_,
):
...
@kopf.on.probe(
    id=application_ingressroutetcp_idx.__name__,
)
async def application_ingressroutetcp_probe(
    application_ingressroutetcp_idx: kopf.Index,
    **_,
):
    return {
        namespace: {
            o["metadata"]["name"]: o
            for o in application_ingressroutetcp_idx[namespace]
        }
        for namespace in application_ingressroutetcp_idx
    }
...
async def application_ingressroutetcp(
    namespace,
    body,
    patch,
    **kwargs,
    ...
    body = (
        load(
            env.get_template("ingressroutetcp.yaml.j2").render(
                namespace=namespace,
                selector=label.but(
                    LABEL_CONTROLLER,
                    LABEL_INSTANCE,
